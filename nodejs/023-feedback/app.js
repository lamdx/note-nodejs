...
// /comments?name=jack&message=hello world
// 对于这种表单提交的请求路径，由于其中具有用户动态填写的内容
// 所以你不可能通过完整的 url 路径去判断来处理这个请求
// 
// 结论：对于我们来讲，其实只需要判定，如果你的请求路径是 /comments 的时候，
// 那我就认为你提交表单的请求过来了
...
http
    ...
    .createServer(function(req, res) {
        ...
        // http://localhost:3000/comments?name=hello&message=12
        ...
        if (pathname === '/') {
            ...
            // 注意：这个时候无论 /comments?xxx 之后是什么，我都不用担心了，
            // 因为我的 pathname 是不包含 ? 之后的那个路径
            // 一次请求对应一次响应，随着响应的结束，这次的请求也就结束了
            // res.end(JSON.stringify(parseObj.query))

            // 我们已经使用 url 模块的 parse 方法把请求路径中的查询字符串给解析
            // 成一个对象了, 所以接下来要做的就是：
            ...
            //        setHeader
            // 如果客户端发现收到服务器的响应的状态码是 302 
            // 就会自动去响应头中找 Location ，然后对该地址发起新的请求
            // 所以你就能看到客户端自动跳转了
            res.statusCode = 302
